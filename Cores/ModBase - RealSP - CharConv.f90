...
!** ACCESSIBLE SPECIFICATIONS OF MODULE DATA, SUBROUTINES OR FUNCTONS
    ! Real-To-String
    PUBLIC :: RealSP_ToString_DragonBox
    PUBLIC :: RealSP_ToString_Ryu
    PUBLIC :: RealSP_ToString_Schubfach
    ! Real-From-String
    PUBLIC :: RealSP_FromString_FastFloat
    PUBLIC :: RealSP_FromString_LibC
    PUBLIC :: RealSP_FromString_YY
...
!** MODULE PARAMETERS:
    ...
    ! kind
    INTEGER(KIND=I4B), PARAMETER  :: RealKind         = 4
    ! binary-precision number of bit
    INTEGER(KIND=I4B), PARAMETER  :: BinaryPrecision  = 24
    INTEGER(KIND=I4B), PARAMETER  :: TotalBits        = 32
    INTEGER(KIND=I4B), PARAMETER  :: SignBits         = TotalBits - 1                                             ! 31
    INTEGER(KIND=I4B), PARAMETER  :: SignificandBits  = BinaryPrecision - 1                                       ! 23
    INTEGER(KIND=I4B), PARAMETER  :: ExponentBits     = TotalBits - BinaryPrecision                               ! 8
    INTEGER(KIND=I4B), PARAMETER  :: MaxExponent      = SHIFTL(1, ExponentBits) - 1                               ! 255
    INTEGER(KIND=I4B), PARAMETER  :: ExponentBias     = SHIFTL(1, ExponentBits - 1) - 1                           ! 127
    INTEGER(KIND=I4B), PARAMETER  :: DecimalPrecision = INT((SignificandBits * LB2To10_M1) / LB2To10_M2, KIND=I4B)      ! 6
    INTEGER(KIND=I4B), PARAMETER  :: DecimalRange     = INT(((ExponentBias - 1) * LB2To10_M1) / LB2To10_M2, KIND=I4B)   ! 37
    INTEGER(KIND=I4B), PARAMETER  :: MaxDecimalConversionDigits = 112
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! +++ masking parameters +++
    INTEGER(KIND=I4B), PARAMETER  :: SigHidBitMask    = SHIFTL(1, SignificandBits)
    INTEGER(KIND=I4B), PARAMETER  :: SignificandMask  = SigHidBitMask - 1
    INTEGER(KIND=I4B), PARAMETER  :: SignMask         = SHIFTL(1, SignBits)
    INTEGER(KIND=I4B), PARAMETER  :: ExponentMask     = NOT(IOR(SignMask, SignificandMask))
    INTEGER(KIND=I4B), PARAMETER  :: ExpMantMask      = SignificandMask + ExponentMask        ! = NOT(SignMask)
    INTEGER(KIND=I4B), PARAMETER  :: QuietNaNMask     = SHIFTL(1, SignificandBits - 1)
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    INTEGER(KIND=I4B), PARAMETER  :: MAX_NUM_DIGITS = 150
    ...
    INTEGER(KIND=I4B), PARAMETER  :: BigUIntBits = 576
    ! the (fixed) capacity of a BigUInt
    INTEGER(KIND=I4B), PARAMETER  :: BigCapacity = BigUIntBits / DigitBits   ! = 9
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    INTEGER(KIND=I4B), PARAMETER  :: DivBase      = 10
    INTEGER(KIND=I4B), PARAMETER  :: MaxDivbyBase = 429496729
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! +++ number parameters +++
    INTEGER(KIND=I4B), PARAMETER  :: TwoUInt   = 2
    INTEGER(KIND=I4B), PARAMETER  :: ThreeUInt = 3
    INTEGER(KIND=I4B), PARAMETER  :: FourUInt  = 4
    INTEGER(KIND=I4B), PARAMETER  :: FiveUInt  = 5
    INTEGER(KIND=I4B), PARAMETER  :: TenUInt   = 10
    INTEGER(KIND=I4B), PARAMETER  :: FortyUInt = 40
    INTEGER(KIND=I4B), PARAMETER  :: HundredUInt     = 100
    INTEGER(KIND=I4B), PARAMETER  :: TenThousandUInt = 10000
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! ----------------------------------------------------
    ...
    ! --------------------------------------------------
    INTEGER(KIND=I4B), PARAMETER  :: LowBits = TotalBits - SignificandBits - 3                                ! = 6
    ! The halfway constant is used to check if the bits that will be shifted away intially are all 1.
    INTEGER(KIND=I4B), PARAMETER  :: HalfWay = SHIFTL(1, LowBits) - 1                                         ! = 63
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! --------------------------------------------------
    ! -----   Clinger-Algorithm' parameters   -----
    ! --------------------------------------------------
    INTEGER(KIND=I4B)             :: Idx
    REAL(KIND=SP),     PARAMETER  :: Powers_Of_Ten(0:10)  = [(10.0E0**Idx, Idx = 0, 10)]
    INTEGER(KIND=I4B), PARAMETER  :: Num_Exact_Pow10 = 10
    INTEGER(KIND=I4B), PARAMETER  :: Num_Mantissa_Digits = 7
    REAL(KIND=SP),     PARAMETER  :: Max_Exact_Integer = 16777215.0_SP
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    INTEGER(KIND=I4B), PARAMETER  :: Exponent_UppBound =  39      ! = 38 + 1
    INTEGER(KIND=I4B), PARAMETER  :: Exponent_LowBound = -54      ! = (-45) - 9
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! -----------------------------------------------
    ! -----   Dragonbox-Algorithm' parameters   -----
    ! -----------------------------------------------
    ! parameters for main routine
    INTEGER(KIND=I4B), PARAMETER  :: Kappa = 1
    INTEGER(KIND=I4B), PARAMETER  :: Big_Divisor = 10**(Kappa+1)                  ! 100
    INTEGER(KIND=I4B), PARAMETER  :: Small_Divisor = Big_Divisor / 10             ! 10
    INTEGER(KIND=I4B), PARAMETER  :: Half_Small_Divisor = Small_Divisor / 2       ! 5
    INTEGER(KIND=I4B), PARAMETER  :: Divisibility_Check_By_5_Threshold = 39
    INTEGER(KIND=I4B), PARAMETER  :: Case_Fc_Pm_Half_Lower_Threshold = -1
    ...
    INTEGER(KIND=I4B), PARAMETER  :: Shorter_Interval_Tie_Lower_Threshold = -35
    INTEGER(KIND=I4B), PARAMETER  :: Shorter_Interval_Tie_Upper_Threshold = -35
    ! parameters for Is_Divisible_By_Pow10 routine
    INTEGER(KIND=I4B), PARAMETER  :: Info_Shift_Amount = 16
    ...
    INTEGER(KIND=I8B), PARAMETER  :: DivM = 1374389535_I8B
    INTEGER(KIND=I4B), PARAMETER  :: DivS = 37
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! -----------------------------------------------
    ! -----   Ryu-Algorithm' parameters   -----
    ! -----------------------------------------------
    INTEGER(KIND=I4B), PARAMETER  :: BitsPerPow5 = 64
    INTEGER(KIND=I4B), PARAMETER  :: MaxExp_ModInv5 = 13
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! -----------------------------------------------
    ! -----   Schubfach-Algorithm' parameters   -----
    ! -----------------------------------------------
    INTEGER(KIND=I4B), PARAMETER  :: Pow10_Min_Exact_Exp = 0
    INTEGER(KIND=I4B), PARAMETER  :: Pow10_Max_Exact_Exp = 27
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! -----------------------------------------------
    ...
    INTEGER(KIND=I4B), PARAMETER  :: Max_Exponent_Round_To_Even = 10
    INTEGER(KIND=I4B), PARAMETER  :: Min_Exponent_Round_To_Even = -17
    ...
    INTEGER(KIND=I4B), PARAMETER  :: BitMask        = SHIFTL(1, LowBits) - 1          ! = Halfway
    INTEGER(KIND=I4B), PARAMETER  :: BitMaskMinus1  = BitMask - 1
    INTEGER(KIND=I4B), PARAMETER  :: AddRound       = SHIFTL(1, ExponentBits - 1)
    INTEGER(KIND=I4B), PARAMETER  :: MaxUInt        = INT(Z'FFFFFFFF', KIND=I4B)
    INTEGER(KIND=I4B), PARAMETER  :: FpRawInf       = INT(Z'7F800000', KIND=I4B)            ! = ExponentMask
    INTEGER(KIND=I4B), PARAMETER  :: MaxExpBin      = 128
    INTEGER(KIND=I4B), PARAMETER  :: MinExpBin      = -125
    INTEGER(KIND=I4B), PARAMETER  :: UIntSafeDigits = 9
    INTEGER(KIND=I4B), PARAMETER  :: MaxDecDigits   = MaxDecimalConversionDigits + 1
    INTEGER(KIND=I4B), PARAMETER  :: MaxMantissa    = SHIFTL(1_I8B, BinaryPrecision)
    ! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE Parsed_Number_Info
        ...
        INTEGER(KIND=I4B)     :: Sig              ! base-10 significand
    ...
    ! ----------------------------------------------------------------------------
    TYPE BinRep
        INTEGER(KIND=I4B)     :: Significand  ! significand/mantissa (M)
...
!DIR$ ATTRIBUTES FORCEINLINE :: Get_Pow10_64Bits
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    INTEGER(KIND=I8B)             :: Pow10    ! the power of ten
...
! empty string
        RealNum = IEEE_VALUE(0.0_SP, IEEE_QUIET_NAN)
...
! Try to parse Inf, maybe with a sign
        ...
        IF (Finish == Q + 2) THEN
            IF ((ToUpper(cStr(Q:Q)) == 'I').AND.(ToUpper(cStr(Q+1:Q+1)) == 'N').AND. &
                ...
                IF (Negative) THEN
                    RealNum = IEEE_VALUE(1.0_SP, IEEE_NEGATIVE_INF)
                ELSE
                    RealNum = IEEE_VALUE(1.0_SP, IEEE_POSITIVE_INF)
                END IF
            ELSE
! Invalid NAN
                RealNum = IEEE_VALUE(0.0_SP, IEEE_SIGNALING_NAN)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    REAL(KIND=SP), INTENT(IN)   :: RealVal
    INTEGER(KIND=I4B)           :: RawVal
...
! set infinity biased exponent
    Exponent = IAND(SHIFTL(INT(MaxExponent, KIND=I4B), SignificandBits), ExponentMask)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    CLASS(HPDecimal),   INTENT(IN)    :: HP
    INTEGER(KIND=I4B),  INTENT(OUT)   :: ResVal
...
!** FLOW
    ...
    DO WHILE ((CurDigit < HP%DecimalPoint).AND.(CurDigit < HP%NumDigits))
        ResVal = ResVal * 10 + INT(HP%Digits(CurDigit), KIND=I4B)
        CurDigit = CurDigit + 1
    END DO
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    TYPE(BigUInt),      INTENT(INOUT)    :: Big  ! a big number (can be 0)
    INTEGER(KIND=I4B),  INTENT(IN)       :: Val  ! an unsigned integer (can be 0)
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    TYPE(BigUInt),            INTENT(INOUT)    :: Big      ! a big number (can be 0)
    INTEGER(KIND=I4B),        INTENT(IN)       :: SigDec   ! significand in base 10
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    ...
    INTEGER(KIND=I4B),                       INTENT(OUT)   :: SigDec   ! significand in base 10
...
!** SUBROUTINE PARAMETER DECLARATIONS:
    ...
    INTEGER(KIND=I4B), PARAMETER  :: FP_Max_Digits = 9

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    INTEGER(KIND=I4B)           :: SigLimit
...
! Compute K and Beta as well as get cached data
    ...
    IF (Minus_K > 0) Pow10 = Pow10 + 1_I8B
...
! when z^(f) == epsilon^(f), or equivalently, when y is an integer
            IF (YMul_IsInteger) THEN
                IF (IAND(SigDec, 1_I4B) /= 0_I4B) SigDec = SigDec - 1_I4B
            END IF
        END IF
    END IF
    ...
    !** SUBROUTINE ARGUMENT DECLARATIONS:
        ...
        INTEGER(KIND=I8B), INTENT(IN)   :: Pow10
        INTEGER(KIND=I4B), INTENT(OUT)  :: ResHi
...
!**************************************************************************
    ...
    !** SUBROUTINE ARGUMENT DECLARATIONS:
        INTEGER(KIND=I4B), INTENT(IN)   :: Two_F
        INTEGER(KIND=I8B), INTENT(IN)   :: Pow10
    ...
    !** FLOW
        ...
        Parity = IAND(SHIFTR(Output, (64 - Beta)), 1_I8B) /= 0_I8B
...
!**************************************************************************
    ...
    ! When tie occurs, choose one of them according to the rule
        IF (Exponent >= Shorter_Interval_Tie_Lower_Threshold .AND. &
            ...
            IF (IAND(SigDec, 1_I4B) /= 0_I4B) SigDec = SigDec - 1_I4B    ! Round to even.
...
!**************************************************************************
    ...
    !** FLOW
        ...
        X = INT(IAND(SHIFTR((Pow10 - SHIFTR(Pow10, (SignificandBits + 2))), &
                            (TotalBits*2 - SignificandBits - 1 - Beta)),    &
                     MaskU32), KIND=I4B)
...
!**************************************************************************
    ...
    !** FLOW
        ...
        Z = INT(IAND(SHIFTR((Pow10 + SHIFTR(Pow10, (SignificandBits + 1))), &
                            (TotalBits*2 - SignificandBits - 1 - Beta)),    &
                     MaskU32), KIND=I4B)
...
!**************************************************************************
    ...
    !** FLOW
        ...
        Y = SHIFTR(INT(IAND(SHIFTR(Pow10, (TotalBits*2 - SignificandBits - 2 - Beta)), &
...
!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    ...
    INTEGER(KIND=I4B)   :: MV, MP, MM
    INTEGER(KIND=I4B)   :: Vr, Vp, Vm
    INTEGER(KIND=I4B)   :: VrDiv10, VpDiv10, VmDiv10
    INTEGER(KIND=I4B)   :: VrMod10, VmMod10
...
! Only one of MP, MV, and MM can be a multiple of 5, if any.
            IF (UMOD(MV, FiveUInt) == 0_I4B) THEN
...
! Step 4: Find the shortest decimal representation in the interval of legal representations.
    ...
    DO WHILE (VpDiv10 .UGT. VmDiv10)
        VmIsTrailingZeros = VmIsTrailingZeros .AND. (VmMod10 == 0_I4B)
    ...
    IF (VmIsTrailingZeros) THEN
        DO WHILE (Mod_By_Pow10Factor(Vm, 10) == 0_I4B)
            VrIsTrailingZeros = VrIsTrailingZeros .AND. (LastRemovedDigit == 0)
            CALL DivMod_By_Pow10Factor(Vr, 10, VrDiv10, VrMod10)
            LastRemovedDigit = VrMod10
            Vr = VrDiv10
    ...
    IF (VrIsTrailingZeros.AND.(LastRemovedDigit == 5).AND.(Mod_By_Pow10Factor(Vr, 2) == 0_I4B)) THEN
...
! We need to take Vr+1 if Vr is outside bounds or we need to round up.
    ...
    !** SUBROUTINE ARGUMENT DECLARATIONS:
        INTEGER(KIND=I4B), INTENT(IN)   :: U, V, W
        INTEGER(KIND=I4B),   INTENT(IN)   :: E5, E2
        INTEGER(KIND=I4B), INTENT(OUT)  :: A, B, C
    ...
    !** FLOW
        ...
        IF ((E5 < Pow10_Min_Exact_Exp).OR.(E5 > Pow10_Max_Exact_Exp)) Pow5 = Pow5 + 1_I8B
...
!**************************************************************************
    ...
    !** SUBROUTINE ARGUMENT DECLARATIONS:
        ...
        INTEGER(KIND=I4B), INTENT(OUT)    :: Quotient
        INTEGER(KIND=I4B), INTENT(OUT)    :: Remainder
    ...
    ! The following algorithm actually can be used for all divisor (Y)
        Quotient  = INT(ToUnsignedLong(X)/ToUnsignedLong(Y), KIND=I4B)
...
!**************************************************************************
    ...
    ! The following algorithm actually can be used for all divisor (Y)
        Remainder = INT(MOD(ToUnsignedLong(X), ToUnsignedLong(Y)), KIND=I4B)
...
!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    INTEGER(KIND=I8B)   :: Pow10
    INTEGER(KIND=I4B)   :: Cb, Cbl, Cbr, Vb, Vbl, Vbr
    INTEGER(KIND=I4B)   :: kExp, hExp, Exp10
    LOGICAL             :: uInside, wInside
    INTEGER(KIND=I4B)   :: Sx, Sx4, Sp
...
! To perform integer multiplications and get upper bits of rounded values
    ...
    IF (IAND(SigBin, 1_I4B) == 0_I4B) THEN
        Lower = Vbl
        Upper = Vbr
    ELSE
        Lower = Vbl + 1_I4B
...
!** FLOW
    ...
    FloatMantissa = REAL(SigDec, KIND=SP)
    Exp10 = ExpDec
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    INTEGER(KIND=I4B),  INTENT(IN)   :: SigDec
    INTEGER(KIND=I4B),  INTENT(IN)   :: ExpDec
    CHARACTER(LEN=*),   INTENT(IN)   :: cStr
    INTEGER(KIND=I4B),  INTENT(IN)   :: Start
    LOGICAL,            INTENT(IN)   :: Truncated
    INTEGER(KIND=I4B),  INTENT(OUT)  :: SigBin
...
!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
!    INTEGER(KIND=I4B)   :: FirstSigBin
!    INTEGER(KIND=I4B)   :: FirstExpBin

!** FLOW

!NOTE : the Eisel-Lemire's algorithm based on LibC appears to be incorrect when the shortest-representation
!       decimal string is halfway (i.e. between the correct value and its neighber, lower or upper).
!       so the one based Fast_Double_Parser is used here
    ! check whether to use Lemire's fast path
    IF ((.NOT.Truncated).AND.(ExpDec > MinExpFastPath).AND.(ExpDec < MaxExpFastPath)) THEN
        ! return if fast path is valid
        IF (D2B_Lemire_FastPath(SigDec, ExpDec, SigBin, ExpBin)) RETURN
    END IF
...
!        FirstSigBin = SigBin
!        FirstExpBin = ExpBin
!        IF (Eisel_Lemire(SigDec + 1_I4B, ExpDec, SigBin, ExpBin)) THEN
...
!DIR$ ATTRIBUTES FORCEINLINE :: D2B_Lemire_FastPath
    ...
    !** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
        INTEGER(KIND=I8B)     :: Pow10
        INTEGER(KIND=I8B)     :: MulProduct
        INTEGER(KIND=I4B)     :: LZ
        INTEGER(KIND=I4B)     :: Upperbit
        INTEGER(KIND=I4B)     :: Significand
        INTEGER(KIND=I4B)     :: Lower, Upper
        INTEGER(KIND=I4B)     :: Exponent
    ...
    ! because the most significant bit of Significand is 1.
        ...
        Lower = INT(IAND(MulProduct, MaskU32), KIND=I4B)
        Upper = INT(SHIFTR(MulProduct, 32), KIND=I4B)
    ...
    ! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        IF ((IAND(Upper, BitMask) == BitMask).AND.((Lower + Significand) .ULT. Lower)) THEN
            BLOCK
                ! --- declaration ---
                INTEGER(KIND=I4B)   :: Product_Low, Product_High
                INTEGER(KIND=I4B)   :: Product_Middle, Product_Middle1, Product_Middle2
                ! --- execution ---
    ! perform multiplication
                MulProduct = ToUnsignedLong(Significand)*IAND(Pow10, MaskU32)
                Product_Low     = INT(IAND(MulProduct, MaskU32), KIND=I4B)
                Product_Middle2 = INT(SHIFTR(MulProduct, 32), KIND=I4B)
                Product_Middle1 = Lower
                Product_High    = Upper
                Product_Middle  = Product_Middle1 + Product_Middle2
    ! overflow carry
                IF (Product_Middle .ULT. Product_Middle1) Product_High = Product_High + 1_I4B
    ...
    ! This does happen, e.g. with 7.3177701707893310E+15 (for double-precision)
                IF (((Product_Middle + 1_I4B == 0_I4B).AND.(IAND(Product_High, BitMask) == BitMask) &
    ...
    ! floating-point values.
        IF ((Lower == 0_I4B).AND.(IAND(Upper, BitMask) == 0_I4B).AND. &
            (IAND(SigBin, ThreeUInt) == 1_I4B)) THEN
    ...
    !       down (except for small positive powers), almost always should round up.
            ...
            RETURN
        END IF
        SigBin = SHIFTR(SigBin + IAND(SigBin, 1_I4B), 1)
    ...
    !** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
        ...
        INTEGER(KIND=I4B)     :: Mantissa, HighU32
        INTEGER(KIND=I8B)     :: PowTen
        INTEGER(KIND=I8B)     :: FirstApprox, FinalApprox, LowBits, SecondApprox
        INTEGER(KIND=I4B)     :: FinalMantissa
    ...
    ! Wider Approximation
        IF ((IAND(INT(SHIFTR(FirstApprox, 32), KIND=I4B), HalfWay) == HalfWay).AND. &
            (INT(IAND(FirstApprox, MaskU32), KIND=I4B) + Mantissa .ULT. Mantissa)) THEN
            BLOCK
                ! --- declaration ---
                INTEGER(KIND=I4B)   :: Product_Low, Product_High
                INTEGER(KIND=I4B)   :: Product_Middle, Product_Middle1, Product_Middle2
                ! --- execution ---
                ! perform multiplication
                LowBits = ToUnsignedLong(Mantissa)*IAND(PowTen, MaskU32)
                Product_Low     = INT(IAND(LowBits, MaskU32), KIND=I4B)
                Product_Middle2 = INT(SHIFTR(LowBits, 32), KIND=I4B)
                Product_Middle1 = INT(IAND(FirstApprox, MaskU32), KIND=I4B)
                Product_High    = INT(SHIFTR(FirstApprox, 32), KIND=I4B)
                Product_Middle  = Product_Middle1 + Product_Middle2
                ! overflow carry
                IF (Product_Middle .ULT. Product_Middle1) Product_High = Product_High + 1_I4B
    ...
    ! Shifting to 25 bits for singles
        HighU32 = INT(SHIFTR(FinalApprox, 32), KIND=I4B)
        MSB = INT(SHIFTR(HighU32, TotalBits - 1), KIND=I4B)
        FinalMantissa = SHIFTR(HighU32, (MSB + TotalBits - (SignificandBits + 3)))
        Exp2 = Exp2 - IEOR(1, MSB)  ! same as NOT(MSB)

    ! Half-way ambiguity
        IF ((INT(IAND(FinalApprox, MaskU32), KIND=I4B) == 0).AND. &
            (IAND(HighU32, HalfWay) == 0).AND.(IAND(FinalMantissa, 3) == 1)) THEN
            Valid = FalseVal
            RETURN
        END IF
...
!******************************************************************************
    ...
    ! Check if by shifting right we've caused this to round to a normal number.
            IF (SHIFTR(FinalMantissa, SignificandBits) /= 0_I4B) THEN
                Exp2 = Exp2 + 1
            END IF
        END IF

    ! Check if rounding added a bit, and shift down if that's the case.
        IF (FinalMantissa == SHIFTL(INT(2, KIND=I4B), SignificandBits)) THEN
            FinalMantissa = SHIFTR(FinalMantissa, 1)
            Exp2 = Exp2 + 1
...
!**************************************************************************
    ...
    ! check for special cases (may not be needed since it is taken care of in the caller?)
        IF ((W == 0_I4B) .OR. (Q < Smallest_Power_of_Ten)) THEN
            E = 0
            M = 0_I4B
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    INTEGER(KIND=I4B),  INTENT(IN) :: SigDec   ! significand, base 10
    ...
    TYPE(StringAux),    INTENT(IN) :: Aux      ! auxiliary string information
    INTEGER(KIND=I4B)              :: RawFP    ! floating point number as an unsigned integer
...
!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    INTEGER(KIND=I4B)   :: SigBin   ! significand, base 2
...
! then, add exponent bits
            RawFP = IOR(RawFP, SHIFTL(INT(ExpBin, KIND=I4B), SignificandBits))
...
!DIR$ ATTRIBUTES FORCEINLINE :: D2B_YY_FastPath
    ...
    !** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
        ...
        INTEGER(KIND=I8B)   :: FirstProduct, SecondProduct
        INTEGER(KIND=I4B)   :: Hi, Lo, Hi2
        INTEGER(KIND=I4B)   :: Sig1, Add, Bits
    ...
    ! normalize and multiply
        ...
        Lo = INT(IAND(FirstProduct, MaskU32), KIND=I4B)
    ...
    ! more lower bits with another multiplication.
            ...
            Hi2 = INT(SHIFTR(SecondProduct, 32), KIND=I4B)
            Add = Lo + Hi2
            IF ((Add /= 0_I4B).AND.(Add /= MaxUInt)) THEN
    ...
    ! The 'Hi' cannot be MaxUInt, so it will not overflow.
                IF ((Add .ULT. Lo).OR.(Add .ULT. Hi2)) Hi = Hi + 1_I4B
                Exact = TrueVal
            END IF
        END IF
        IF (Exact) THEN
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    INTEGER(KIND=I4B),  INTENT(IN) :: SigDec   ! significand
...
! "Do It Yourself Floating Point"
    TYPE Diy_Fp
        INTEGER(KIND=I4B)   :: Sig      ! significand
...
!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    ...
    INTEGER(KIND=I4B)   :: FpErr, PrecisionBits, HalfWay
    INTEGER(KIND=I4B)   :: Bits
...
! initialize
    ...
    IF (Aux%Truncated) THEN
        FpErr = INT(ERR_ULP / 2, KIND=I4B)
! round up if the next digit after the cut is more than or equal to 5
        IF ((IACHAR(cStr(Aux%SigCut:Aux%SigCut))-A0) >= 5) Fp%Sig = Fp%Sig + 1_I4B
    ELSE
        FpErr = 0_I4B
    END IF
...
! multiply and add error
    ...
    IF (FpErr == 0_I4B) THEN
        FpErr = FpErr + INT(ERR_CACHED_POW + ERR_MUL_FIXED, KIND=I4B)
    ELSE
...
! precision digits count
    ...
    IF (PrecisionDigitsCount + ERR_ULP_LOG >= DIY_SIG_BITS) THEN
        BLOCK
            ...
            Fp%Exp = Fp%Exp + Shr
            FpErr  = SHIFTR(FpErr, Shr) + INT(1 + ERR_ULP, KIND=I4B)
            PrecisionDigitsCount = PrecisionDigitsCount - Shr
        END BLOCK
    END IF

! half way
    PrecisionBits = IAND(Fp%Sig, (SHIFTL(1_I4B, PrecisionDigitsCount) - 1_I4B))
...
! falls midway, round to even
    ...
    !** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
        INTEGER(KIND=I8B)     :: Pow10
        INTEGER(KIND=I4B)   :: Sig_Ext

    !** FLOW

        Pow10   = Get_Pow10_64Bits(Exp10)
        Fp%Sig  = INT(SHIFTR(Pow10, 32), KIND=I4B)
        Sig_Ext = INT(IAND(Pow10, MaskU32), KIND=I4B)
    ...
    !** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
        INTEGER(KIND=I4B)   :: Hi, Lo
        INTEGER(KIND=I8B)   :: Product
    ...
    !** SUBROUTINE ARGUMENT DECLARATIONS:
        ...
        INTEGER(KIND=I4B)           :: Val

    !** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
        INTEGER(KIND=I4B)   :: Sig
    ...
    ! normal
            ...
            Val = IOR(SHIFTL(INT(Exp, KIND=I4B), SignificandBits), IAND(Sig, SignificandMask))
...
!** SUBROUTINE ARGUMENT DECLARATIONS:
    INTEGER(KIND=I4B), INTENT(IN) :: X    ! X <= 10**10
    INTEGER(KIND=I4B), INTENT(IN) :: P    ! 1 <= P <= 8
    INTEGER(KIND=I4B)             :: Y

!** SUBROUTINE PARAMETER DECLARATIONS:
! Parameters for division by power of 10 applicable for N <= 10 digits
! (i.e. used for division of the 'Significand')
    INTEGER(KIND=I8B), PARAMETER  :: MagicM(1:8) = [                        &
        INT(Z'0000000333333334', KIND=I8B), INT(Z'000000028F5C28F6', KIND=I8B), &
        INT(Z'00000004189374BD', KIND=I8B), INT(Z'0000000346DC5D64', KIND=I8B), &
        INT(Z'000000029F16B11D', KIND=I8B), INT(Z'0000000431BDE82E', KIND=I8B), &
        INT(Z'000000035AFE5358', KIND=I8B), INT(Z'00000002AF31DC47', KIND=I8B)]
    INTEGER(KIND=I4B), PARAMETER  :: MagicS(1:8) = [37, 40, 44, 47, 50, 54, 57, 60]

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    INTEGER(KIND=I8B)     :: Multiplier
    INTEGER(KIND=I4B)     :: Shift
...
!DIR$ ATTRIBUTES INLINE :: WRITE_RealSP
...
!   that guarantees an error-free write-read cycle.)
    INTEGER(KIND=I4B), PARAMETER  :: H = 9
...
! The first powers of 10. The last entry must be 10^H.
    INTEGER(KIND=I4B)             :: I
    INTEGER(KIND=I8B), PARAMETER  :: Pow10(0:H) = [(10**I, I = 0, H)]
! Used for left-to-tight digit extraction.
    INTEGER(KIND=I4B), PARAMETER  :: MASK_28 = SHIFTL(1, 28) - 1

!** SUBROUTINE INTERNAL VARIABLE DECLARATIONS:
    INTEGER(KIND=I4B)     :: F
    INTEGER(KIND=I4B)     :: E, HF, LF
...
!DIR$ ATTRIBUTES FORCEINLINE :: ToChar_Plain_Without_LZ
    ...
    ! For n = 17, m = 8 the table in section 10 of [3] leads to:
        Y = INT(SHIFTR(UMul128_Upper64(SHIFTL(INT(L+1, KIND=I8B), 28), M178), S178), KIND=I4B) - 1
        I = 1
        DO WHILE (I < E)
            T = 10*Y
    ! append digit
...
!DIR$ ATTRIBUTES FORCEINLINE :: Write_U32_8_Digits
...
END MODULE ModBase_RealSP_CharConv
