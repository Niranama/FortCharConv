...
!** PURPOSE OF THIS MODULE:
    ! contains parameters commonly used
...
!** MODULE PARAMETERS:
    ...
    INTEGER,          PARAMETER :: FP = DP          ! kind of default floating point numbers
    ! angle unit flag
    ...
    CHARACTER(LEN=*), PARAMETER :: LibName = "XPF Collections"  ! library name
...
!** DERIVED TYPE DEFINITIONS
    ...
    TYPE UserParam
        INTEGER(KIND=IP)                :: NR
        INTEGER(KIND=IP)                :: NI
        REAL(KIND=FP),    ALLOCATABLE   :: RPar(:)
        INTEGER(KIND=IP), ALLOCATABLE   :: IPar(:)
    END TYPE UserParam
	TYPE WorkSpace
        INTEGER(KIND=IP)                :: LRW
        INTEGER(KIND=IP)                :: LIW
        REAL(KIND=FP),    ALLOCATABLE   :: RVar(:)
        INTEGER(KIND=IP), ALLOCATABLE   :: IVar(:)
    END TYPE WorkSpace
	TYPE SharePar(NR,NI)
        INTEGER(KIND=IP), LEN   :: NR,NI
        REAL(KIND=FP)           :: RPar(NR)
        INTEGER(KIND=IP)        :: IPar(NI)
    END TYPE SharePar
    TYPE SaveVar(NR,NI,NL)
        INTEGER(KIND=IP), LEN   :: NR,NI,NL
        REAL(KIND=FP)           :: RVar(NR)
        INTEGER(KIND=IP)        :: IVar(NI)
        LOGICAL                 :: LVar(NL)
    END TYPE SaveVar
...
!** MODULE VARIABLE DECLARATIONS:
    LOGICAL     :: Is_XPFC_Library      = TrueVal
    LOGICAL     :: Is_FMLib_Initialized = FalseVal
    LOGICAL     :: Is_MPFun_Initialized = FalseVal
